...
var DinnerModel = function() {
	...
	var guests =4; //The number of guests selected from the beginning
	var currentDish = 1; //The current dish showed
	var currentType = 'starter'; //The current type of dishes showed in the beginning
	var searchWord = [];
	this.Word = ""; //searched word
	var singleRecipe=[];
...
//notifyObservers();
	...
	this.getAllDishes = function (Keyword) {
        ...
        var apiKey = "dvx9vce85UWwaqam4cY8J3ViaCbdjyuO";
        
		var titleKeyword = Keyword;
        var url = "http://api.bigoven.com/recipes?pg=1&rpp=16"
			if (titleKeyword != ""){
...
//Adds an observer
	...
	var notifyObservers = function(args){ //Goes through all views and run the update function
		for (n=0;n<observers.length;n++){
			observers[n].updateView(args);
		}
	}
